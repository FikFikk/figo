<template>
  <div class="max-w-7xl mx-auto p-4 md:p-6 text-slate-800 dark:text-slate-200 min-h-[100dvh] pb-36 pt-20 md:pt-24 transition-colors duration-300">
    ...
    <div v-if="!selectedDoc" class="max-w-5xl mx-auto w-full mb-8 space-y-4">
      <div class="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
        ...
        <div class="relative flex-grow max-w-xl">
          ...
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Cari artikel, tokoh, atau ajaran..."
            class="w-full pl-11 pr-4 py-3 bg-white dark:bg-[#121212] border border-slate-200 dark:border-slate-800 rounded-2xl text-sm focus:outline-none focus:border-emerald-500 dark:focus:border-emerald-700 transition-colors shadow-sm text-slate-800 dark:text-slate-100"
          />
          <button v-if="searchQuery" @click="searchQuery = ''" class="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>

        <!-- Sorting Options -->
        <div class="flex items-center gap-2 bg-white dark:bg-[#121212] border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-3 shadow-sm shrink-0 self-end md:self-auto">
      ...
      <div class="flex items-center gap-2 overflow-x-auto pb-2 -mx-4 px-4 md:mx-0 md:px-0 scrollbar-none">
        <button
          v-for="cat in categories"
          :key="cat"
          @click="selectedCategory = cat"
          class="px-4 py-2 text-xs font-semibold rounded-full border transition-all whitespace-nowrap"
          :class="selectedCategory === cat 
            ? 'bg-emerald-600 dark:bg-emerald-950 border-emerald-500 dark:border-emerald-800 text-white dark:text-emerald-300 shadow-sm'
            : 'bg-white dark:bg-[#121212] border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'"
        >
          {{ cat }}
        </button>
      </div>
    ...
    <!-- LIST GRID ARTIKEL -->
    <div v-if="!selectedDoc" class="max-w-5xl mx-auto w-full">
      <div v-if="paginatedDocuments.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      ...
      <!-- Empty State -->
      <div v-else class="flex flex-col items-center justify-center py-20 text-center bg-white dark:bg-[#121212] border border-slate-200 dark:border-slate-800 rounded-2xl p-8">
        ...
        <h3 class="text-base font-semibold text-slate-700 dark:text-slate-300 mb-1">Artikel Tidak Ditemukan</h3>
        <p class="text-xs text-slate-400 dark:text-slate-500">Coba ganti kata kunci pencarian atau kategori filter Anda.</p>
      </div>
...
<!-- MOBILE BOTTOM SHEET OVERLAY FOR SOROTAN -->
      ...
      <div class="lg:col-span-9 order-2">
        ...
        <div id="baca-top" v-if="currentBab" :key="currentPage" class="bg-[#FDFCF8] dark:bg-[#1C1C1E] border border-[#F2F0E9] dark:border-slate-800/60 p-6 md:p-10 rounded-2xl min-h-[50vh] shadow-sm dark:shadow-xl animate-in fade-in duration-300 relative z-10">
          ...
          <!-- Email Hubungi Kami -->
          <div v-if="currentPage === totalPages" class="mt-12 pt-6 border-t border-[#F2F0E9] dark:border-slate-800/40 text-center">
            <p class="text-xs text-slate-400 dark:text-slate-500 font-serif">
              Apabila terdapat kesalahan penulisan isi, kekeliruan kutipan, atau ingin menanyakan terkait sumber data, silakan kirim email ke <a href="mailto:figo@fikfikk.my.id" class="text-emerald-500 hover:text-emerald-600 dark:text-emerald-400 underline transition-colors">figo@fikfikk.my.id</a>.
            </p>
          </div>
        </div>
...
const sortedDocuments = computed(() => {
  ...
  if (searchQuery.value.trim()) {
    ...
    docs = docs.filter(d => 
      ...
      d.tokoh.toLowerCase().includes(q) || 
  ...
  if (selectedCategory.value !== 'Semua') {
    docs = docs.filter(d => d.kategori === selectedCategory.value)
  }
...
const documents = ref([
  {
    ...
    tags: ['Turots Klasik', 'Karomah Mukasyafah', 'Manuskrip 1252 H'],
    deskripsi: 'Kajian kritis silsilah nasab Walisongo, pelacakan manuskrip asli 1252 H, fikih As-Silah dan Al-Matnus Syarif, serta karomah Syaichona Kholil.',
    url: '/dataset/syaichona_kholil_bangkalan.json',
    data: null
  },
  {
    id: 'sri_brahmaraja_genggadhara',
    judul: 'Paduka Sri Brahmaraja Genggadhara (Prasasti Petak & Jiyu)',
    tokoh: 'TONI ANTONIPUTRA & SIWI SANG',
    kategori: 'Babad & Sejarah',
    tags: ['Telaah Prasasti', 'Pralaya 1478 M', 'Girindrawardhana'],
    deskripsi: 'Artikel kajian mendalam berdasarkan lima prasasti bertarikh 1486 M tentang saksi kunci dan arsitek politik kembalinya trah Sang Sinagara ke tahta Majapahit.',
    url: '/dataset/sri_brahmaraja_genggadhara.json',
    data: null
  },
  {
