    ...
    >
      <div v-if="result" class="space-y-4 font-normal">
        ...
        <div class="rounded-xl p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4" :class="resultHeaderClass">
          <div class="flex items-center gap-3 flex-1">
            ...
            <div>
              ...
              <p class="text-sm opacity-80 font-medium">{{ result.summary }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3">
          <div class="rounded-xl p-2.5 md:p-3 text-center border" :class="isDark ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'">
            <div class="flex items-center justify-center gap-1 mb-1">
              ...
              <span class="text-[9px] md:text-[10px] font-black uppercase tracking-widest opacity-50">SSL</span>
            </div>
            <p class="text-[11px] md:text-xs font-bold" :class="result.ssl ? 'text-green-500' : 'text-red-500'">
        ...
        <div v-if="result.threats.length > 0">
          <h3 class="font-headline font-bold text-[10px] uppercase tracking-[0.2em] mb-3 opacity-50">
            Findings ({{ result.threats.length }})
          </h3>
          <div class="space-y-2">
            <div
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  <span class="px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-tighter" :class="severityBadgeClass(threat.severity)">
                    {{ threat.severity }}
                  </span>
                </div>
                <p class="text-[11px] opacity-70 leading-relaxed font-medium">{{ threat.description }}</p>
              </div>
            </div>
          </div>
        </div>
...
const levelLabel = computed(() => {
  ...
  if (l === 'safe') return '✅ Verified Safe'
  if (l === 'warning') return '⚠️ Potential Risks'
...
function threatRowClass(sev: string): string {
  switch (sev) {
    case 'critical': return isDark.value ? 'bg-red-500/8 text-red-400 border-red-500/15' : 'bg-red-50 text-red-800 border-red-100'
    case 'high': return isDark.value ? 'bg-orange-500/8 text-orange-400 border-orange-500/15' : 'bg-orange-50 text-orange-800 border-orange-100'
    case 'medium': return isDark.value ? 'bg-yellow-500/8 text-yellow-400 border-yellow-500/15' : 'bg-yellow-50 text-yellow-800 border-yellow-100'
    case 'low': return isDark.value ? 'bg-blue-500/8 text-blue-400 border-blue-500/15' : 'bg-blue-50 text-blue-800 border-blue-100'
    default: return isDark.value ? 'bg-white/5 text-gray-400 border-white/5' : 'bg-slate-50 text-slate-600 border-slate-100'
  }
}
