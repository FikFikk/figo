  ...
  >
    ...
    <div class="p-5 font-mono">
      ...
      <div v-else-if="loading" class="space-y-3">
        <div class="h-16 border animate-pulse" :class="isDark ? 'bg-neutral-900/40 border-neutral-800' : 'bg-neutral-100 border-neutral-200'"></div>
        <div class="grid grid-cols-2 gap-2">
          <div v-for="i in 4" :key="i" class="h-20 border animate-pulse" :class="isDark ? 'bg-neutral-900/40 border-neutral-800' : 'bg-neutral-100 border-neutral-200'"></div>
        </div>
      </div>
      ...
      <div v-else-if="insightData" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x border"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
            <template v-for="(item, idx) in categoryData" :key="idx">
              <div v-if="item.status !== 'NA'" 
                class="p-3.5 border flex gap-3 relative overflow-hidden transition-all"
                ...
                <div class="flex-1 min-w-0 pl-1">
                   <div class="flex justify-between items-start gap-2 mb-1">
                     ...
                     <span class="text-[8px] font-mono font-bold px-1.5 py-0.5 border"
                       :class="item.status === 'GOOD' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/30' : item.status === 'BAD' ? 'bg-red-500/10 text-red-500 border-red-500/30' : 'bg-neutral-800 text-neutral-400 border-neutral-700'"
                     >[ {{ item.status }} ]</span>
                   </div>
                   ...
                   <div class="flex items-center gap-2 text-[9px] font-mono" v-if="item.value !== undefined && item.benchmark !== undefined">
                      <span class="px-2 py-0.5 border" :class="isDark ? 'bg-neutral-900 border-neutral-800 text-neutral-300' : 'bg-neutral-50 border-neutral-200 text-neutral-700'">
                        VAL: {{ item.value }}
                      </span>
                      <span class="px-2 py-0.5 border opacity-60" :class="isDark ? 'border-neutral-800' : 'border-neutral-200'">
